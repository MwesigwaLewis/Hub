...
def login_required(f):
    ...
    Wrapped in try/except so a transient DB hiccup can never escape as an
    unhandled exception (Flask's default 500 is an HTML page, not JSON,
    which breaks any frontend expecting r.json() to succeed)."""
    @wraps(f)
