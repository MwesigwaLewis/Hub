...
def init_db():
    ...
    # read_at: set the first time this user has ever viewed the message (via
    #   auto-popup OR a manual tap) — drives the bold/dot in the message tile
    #   and inbox modal.
    # popup_count / last_popup_login_count: track the login-triggered
    #   homepage auto-popup independently of read_at. Each announcement is
    #   allowed to auto-popup for a user's first 2 *logins* since it existed
    #   for them (see /api/messages/popup) — capped by popup_count, and
    #   last_popup_login_count stops it firing more than once within the
    #   same login session (users.login_count only advances on POST /login).
    cur.execute("""
        CREATE TABLE IF NOT EXISTS message_reads (
            ...
            last_popup_login_count  INTEGER,
